        ...
        "properties": {
            ...
            "format": {
                ...
                "enum": ["obj",     # Wavefront OBJ (.obj)
                         ...
                         "custom_drc",# Custom compressed (.drc)
...
def compute_brick_labelcounts(brick_df, subset_labels, export_labelcounts, output_dirs):
    ...
    for row in brick_df.itertuples():
        ...
        inner_vol, inner_counts = np.unique(inner_vol, return_counts=True) #use same name to save memory?
        label_counts = pd.Series(inner_counts,index=inner_vol)
        del inner_vol
        del inner_box
...
def serialize_mesh(sv, mesh, path=None, fmt=None, log=True):
    ...
    try:
        if fmt in ["ngmesh","obj","drc"]:
            return mesh.serialize(path, fmt)
        else:
            return serialize_custom_drc(sv, mesh, path)
    except:
        logger = logging.getLogger(__name__)
